 ...
 */
function uploadFileToS3(fileBuffer: Buffer, fileName: string, bucketName: string, folderPath: string) {
    const fullFilePath = `${folderPath}/${fileName}`; // Build the full file path with the folder and file name
    const params = {
        Bucket: bucketName,
        Key: fullFilePath,
        Body: fileBuffer,
 ...
 */
function deleteFileFromS3(fileName: string, bucketName: string, folderPath: string) {
